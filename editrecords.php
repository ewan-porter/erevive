...
<head>
    ...
    <title>eRevive - Edit Records</title>
    ...
    <main class=" flex-fill d-flex align-items-center justify-content-center">
        <div class="set-width">
            ...
            <div class="container form-container">
                <form action="editrecordsprocess.php" method="POST" name="display-records">
...
if (!isset(($_POST['editSearch']))) {


// getting db info from logged in user
$adminQuery = "SELECT * FROM users WHERE username = '$user'";
    ...
    if ($result->num_rows === 1) {
        ...
        //checking if user has admin prights
    ...
    if ($checkAdmin == 0) {
        ...
        // showing only records uploaded by logged in user if they do not have admin rights
        $sql = "SELECT item_name, brand, item_condition, cost, item_description, item_image, id FROM products WHERE user = '$user'";
    } else {
       
        // showing all records to admin suers
...
// outputting records
    while ($row = $result->fetch_assoc()) {
        ...
        echo '  <div class="mt-2 col-md-12 d-flex justify-content-center">
                    <div class="px-2">
                    <a href="deleterecord.php?recordID=' . $row["id"] . '"><input type="submit" class="btn btn-primary btn-lg cancelBtn hvr-grow" value="Delete"
                        id="delete"></a>
                    </div>
                    <div class="px-2">
...
echo "<p>Added products will appear here. You currently have no added records.";
echo "</div>";
}
mysqli_close($conn);
